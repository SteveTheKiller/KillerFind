...
// KillerFind's own PowerShell prompt: where the script lives, and how it reaches a shell.
//
...
// inside KillerFind and nowhere else. No file the user maintains is written to, and their
...
// newer KillerFind drops its version beside it as KillerPrompt.default.ps1, so an upgrade can
...
namespace KillerFind
{
    ...
    {
        private const string PromptResource = "KillerFind.Shell.KillerPrompt.ps1";
        ...
        {
            // Set KF_PROMPT=0 to opt out for good. Checked here rather than in the script so
            // opting out also skips the unpack and the command line stays clean.
            if (Environment.GetEnvironmentVariable("KF_PROMPT") == "0") return string.Empty;
