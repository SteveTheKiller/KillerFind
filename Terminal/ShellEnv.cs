...
// What a shell running inside KillerFind is allowed to know about the window hosting it.
...
// KillerFind owns both the palette and the shell, so the two can agree.
//
// TWO channels, because they answer different questions:
//
//   ENVIRONMENT (KF_*)  - set on THIS process, which ConPty.Launch passes down by inheritance
...
namespace KillerFind
{
    ...
    {
        /// <summary>
        /// Where the live state file lives. Keyed by process id, so two KillerFind windows -
        ...
        private static string ShellStatePath =>
            Path.Combine(
                ...
                "KillerFind", "session", Process.GetCurrentProcess().Id + ".env");
        ...
        // The palette keys a prompt has any business knowing, paired with the KF_ name it is
        ...
        private void RefreshShellEnv()
        {
            ...
            SetVar("KF_SESSION", Process.GetCurrentProcess().Id.ToString());
            SetVar("KF_VERSION", ShellVersion);
            SetVar("KF_ADMIN",   IsElevated ? "1" : "0");
            SetVar("KF_THEME",   Services.ThemeManager.Current.ToString());
            SetVar("KF_ACCENT",  Hex("PrimaryBrush"));
            SetVar("KF_STATE",   state);
