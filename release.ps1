# release.ps1 - KillerFind release workflow
...
# automatically. If KillerFind is ever listed there, copy KillerScan's winget-release.yml
# rather than adding a komac call here, so the two stay consistent.
#
# The site is NOT deployed from here. killerfind.net is a manual Cloudflare Pages drop, so
# this script rewrites find-landing/ with the real release facts and commits it; you drag the
...
Step "Reading version from KillerFind.csproj"
$csproj = Get-Content -Path 'KillerFind.csproj' -Raw
if ($csproj -notmatch '<Version>([0-9]+\.[0-9]+\.[0-9]+)</Version>') {
    Fail 'No <Version>x.y.z</Version> found in KillerFind.csproj'
}
$Version = $Matches[1]
$Tag = "v$Version"
...
if ($csproj -notmatch '<ReleaseDate>([0-9]{4}-[0-9]{2}-[0-9]{2})</ReleaseDate>') {
    Fail 'No <ReleaseDate>yyyy-MM-dd</ReleaseDate> found in KillerFind.csproj'
}
...
if ($msbuild) {
    & $msbuild 'KillerFind.csproj' /t:Publish /p:PublishProfile=FolderProfile /p:Configuration=Release /m /nologo /v:m
} else {
    & dotnet publish 'KillerFind.csproj' /p:PublishProfile=FolderProfile -c Release
}
if ($LASTEXITCODE -ne 0) { Fail 'Build failed' }

$publishDir = 'bin\Release\net48\publish'
$exe = Join-Path $publishDir 'KillerFind.exe'
...
Write-Host "Built KillerFind.exe FileVersion $fileVersion"
...
if ($SkipSign) {
    Write-Host ""
    Write-Host 'SkipSign: KillerFind.exe will be UNSIGNED - do not release this build' -ForegroundColor Red
} else {
    Step "Signing KillerFind.exe"
    ...
    foreach ($tsa in $tsaList) {
        ...
        & $signtool sign /fd sha256 /tr $tsa /td sha256 @certArgs /d 'KillerFind' /du 'https://killerfind.net' /v $exe
...
& (Join-Path $PSScriptRoot 'build\bundle-source.ps1') -ProjectDir $PSScriptRoot -Version $Version -AppName 'KillerFind' -PublishDir $publishDir
$srcZip = Join-Path $publishDir "KillerFind-$Version-src.zip"
...
# the releases page. The updater matches the line starting with KillerFind.exe and takes the
...
Write-Host "Signed KillerFind.exe is $exeMB"
...
$sumsLines = @(
    ('{0,-26} {1}' -f 'KillerFind.exe', $exeHash),
...
# killerfind.net is a MANUAL Cloudflare Pages drop, so nothing here deploys. The hero block
...
Step "Updating find-landing and README release info"
...
$siteDir     = Join-Path (Get-Location).Path 'find-landing'

$indexPath = Join-Path $siteDir 'index.html'
$indexRaw  = [System.IO.File]::ReadAllText($indexPath)
$indexNew  = $indexRaw
$indexNew  = $indexNew -replace '(<span class="k">version</span>&nbsp;<span class="v">)KillerFind v[0-9]+\.[0-9]+\.[0-9]+', ('${1}' + "KillerFind v$Version")
...
$readmeNew  = $readmeRaw -replace '/releases/download/v[0-9]+\.[0-9]+\.[0-9]+/KillerFind-[0-9]+\.[0-9]+\.[0-9]+-src\.zip', "/releases/download/$Tag/KillerFind-$Version-src.zip"
...
if ($DryRun) {
    ...
    Write-Host "  version  : KillerFind v$Version"
...
if ($DryRun) {
    Write-Host "DryRun: would commit and push find-landing + README for v$Version"
} else {
    $siteDirty = git status --porcelain find-landing README.md
    if ($siteDirty) {
        git add find-landing README.md
        ...
        Write-Host "find-landing and README updated to v$Version and pushed"
        Write-Host 'Remember: killerfind.net does NOT auto-deploy. Drag find-landing/ into Cloudflare Pages.' -ForegroundColor Yellow
    } else {
        Write-Host 'find-landing and README already current'
    }
}
...
$notesFile = Join-Path $env:TEMP "KillerFind-$Version-notes.md"
...
if ($DryRun) {
    ...
    Write-Host "  KillerFind.exe ($exeMB)"
...
# --- 9. Tag and push ---
Step "Tagging $Tag"
git tag -a $Tag -m "KillerFind $Tag"
git push origin $Tag
...
gh release create $Tag $exe $srcZip $sumsFile --title "KillerFind $Tag" --notes-file $notesFile --verify-tag
...
if ($Choco) {
    ...
    $nuspec     = Join-Path $chocoDir 'killerfind.nuspec'
    if (-not (Test-Path $nuspec)) {
        Write-Warning 'Skipping Chocolatey - no choco\killerfind.nuspec yet (copy KillerScan''s choco folder as a template).'
    } else {
        ...
        try {
            ...
            choco pack killerfind.nuspec
            if ($LASTEXITCODE -ne 0) { Write-Warning 'choco pack failed' } else {
                $nupkg = Join-Path $chocoDir "killerfind.$Version.nupkg"
...
Write-Host "  site  : find-landing/ is committed and current - drag it into Cloudflare Pages to deploy." -ForegroundColor Yellow
