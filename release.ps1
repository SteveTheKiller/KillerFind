...
<#
.SYNOPSIS
    KillerFind release script: build -> sign -> SHA256 -> print summary.
.DESCRIPTION
    1. Publishes using FolderProfile (net48, win-x64) and zips the source via build\bundle-source.ps1.
    2. Signs KillerFind.exe with your Certum cert via signtool.
    3. Computes and prints the SHA256 for pasting into the landing page.

    Same flow as KillerScan's release.ps1 (family standard).
...
$proj       = Join-Path $PSScriptRoot "KillerFind.csproj"
...
if ($msbuild -eq "dotnet") {
    & dotnet publish $proj /p:PublishProfile=FolderProfile -c Release
} else {
    & $msbuild $proj /t:Publish /p:PublishProfile=FolderProfile /p:Configuration=Release /m /nologo /v:m
}
...
$srcZip = Get-ChildItem $publishDir -Filter "KillerFind-$version-src.zip" -ErrorAction SilentlyContinue | Select-Object -First 1
if ($srcZip) {
...
# -- 4b. SHA256SUMS.txt ---------------------------------------------------------
# Family standard (KillerScan's in-app updater reads this from the release assets).
# KillerFind has no self-updater yet, but shipping the file now means the format is
# already in place when one lands. Just UPLOAD it to the GitHub release next to the exe.
Write-Host "`n==> Writing SHA256SUMS.txt..." -ForegroundColor Cyan
$sumsPath  = Join-Path $publishDir "SHA256SUMS.txt"
$sumsLines = @()
$sumsLines += ("{0,-26} {1}" -f "KillerFind.exe", $hash)
if ($srcZip) {
...
if ($Choco) {   # Chocolatey is opt-in: pass -Choco to pack/push. Default release skips it.
    ...
    if (-not (Test-Path (Join-Path $chocoDir "killerfind.nuspec"))) {
        Write-Host "`n    Skipping Chocolatey -- no choco\killerfind.nuspec yet (copy KillerScan's choco folder as a template)." -ForegroundColor Yellow
    } else {
...
Write-Host   "  KillerFind v$version release artifacts" -ForegroundColor White
Write-Host   "  EXE   : $exe"
...
Write-Host   ""
Write-Host   "  >> UPLOAD to the GitHub release: KillerFind.exe, SHA256SUMS.txt, the -src.zip." -ForegroundColor Yellow
Write-Host   ""
Write-Host   "  Paste SHA256 into:"
Write-Host   "    KillerFind\find-landing\index.html (hero info box, sha256 row)"
