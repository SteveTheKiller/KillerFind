/* KillerFind site - shared chrome behavior (theme, accent, language, easter egg).
...
(function () {
  ...
  function applyAccent(name) {
    ...
    if (neutral) {
      ...
      try { localStorage.setItem('kfind-av', pair[0] + '|' + pair[1]); } catch (e) {}
    } else {
    ...
    try { localStorage.setItem('kfind-accent', name); } catch (e) {}
    updateLogos();
  }
  function updateLogos() {
    ...
    var src = 'brand/killerfind-logo-' + variant + '-' + color + '.svg';
  ...
  function setTheme(name) {
    ...
    try { localStorage.setItem('kfind-theme', name); } catch (e) {}
  ...
  function applyLang(lang) {
    ...
    try { localStorage.setItem('kfind-lang', lang); } catch (e) {}
  }
  ...
  if (verEgg) verEgg.addEventListener('click', function () {
    ...
    if (eggToast) {
      eggToast.textContent = 'grep called. It wants its job back.';
  ...
  try {
    savedTheme  = localStorage.getItem('kfind-theme')  || savedTheme;
    savedAccent = localStorage.getItem('kfind-accent') || savedAccent;
    savedLang   = localStorage.getItem('kfind-lang')   || savedLang;
  } catch (e) {}
