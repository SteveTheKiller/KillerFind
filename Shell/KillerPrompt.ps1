# ============================================================================
#  KillerFind prompt
#  https://killerfind.net
#
#  This is YOUR copy. KillerFind unpacked it once and will never overwrite it,
#  so edit it freely - a later version drops its own beside it as
#  KillerPrompt.default.ps1 rather than replacing this file.
#
#  It is dot-sourced by KillerFind AFTER your $PROFILE has run, only in shells
#  KillerFind starts. Your $PROFILE file is never modified, and no other
#  terminal sees any of this.
#
#  Escape hatches:
#    Restore-Prompt        put your own prompt back, right now, no restart
#    $env:KF_PROMPT = 0    set before launching KillerFind to skip this
#                          entirely
#
...
function Restore-Prompt {
    <#  .SYNOPSIS  Put back the prompt that was in force before KillerFind's.  #>
    if ($script:KFSavedPrompt) {
        Set-Item -Path function:prompt -Value $script:KFSavedPrompt
        Write-Host 'Your prompt is back. Open a new terminal for the KillerFind one.' -ForegroundColor DarkGray
    }
}
...
# ── Theme ───────────────────────────────────────────────────────────────────
# KillerFind writes its live palette to the file named by $env:KF_STATE and
...
# KF_THEME environment variable cannot do that, because a child process only
...
# Truecolor SGR from a #rrggbb string. KillerFind's renderer handles 38;2, so
...
# to single letters: C:\U\s\code\KillerFind. The tail is what you are actually
...
function prompt {
    ...
    if ($env:KF_ADMIN -eq '1') { $line += $accent + '  [ADMIN]' + $reset }
